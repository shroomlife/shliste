...
const computedIsOpen = computed({
  get() {
    return recipeStore.getProductEdit !== null
  },
  set(value) {
    if (!value) {
      recipeStore.setProductEdit(null)
    }
  },
})
...
const computedProductHasId = computed(() => {
  return typeof recipeStore.getProductEdit?.uuid === 'string' && recipeStore.getProductEdit?.uuid.length > 0
})
...
async function onSubmit() {
  recipeStore.updateProduct(state)
  handleClose()
  resetState()
}

watch(() => recipeStore.getProductEdit, (newVal: Product | null) => {
  if (newVal) {
