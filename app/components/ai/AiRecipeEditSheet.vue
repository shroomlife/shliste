...
/**
 * AI-Bearbeitung eines Rezepts — Prompt oder Sprachaufnahme hinein,
 * ZWEI Diff-Vorschauen heraus: Zutaten und Schritte. Nachbau des
 * Android-Sheets (AiEditRecipeSheet.kt) auf der UI-Vorlage von
 * AiEditListSheet: dieselben Phasen, dieselben Diff-Farben, dasselbe
 * Matching über den mitgeschickten `idx` (`computeListDiff` aus diff.ts —
 * Schritte laufen mit `description` als Name hindurch, eine Umformulierung
 * erscheint dadurch als Geändert).
 *
 ...
 * wendet sie über die Mutations-Wege von useRecipeDetail an.
 */
import type { EditedRecipe } from '~/ai/recipeContract'
import type { ListDiffEntry } from '~/ai/diff'
import { computeListDiff, defaultDiffSelection } from '~/ai/diff'
import type { AiRecipeEditApplyPayload, RecipeIngredientSource, RecipeStepSource } from '~/ai/recipeDiff'
import {
...
import { EDIT_RECIPE_PHRASES } from '~/ai/phrases'
import { requestEditRecipe, requestVoiceEditRecipe } from '~/ai/transport'
import type { AiResult } from '~/ai/transport'
import { formatRecordingDuration } from '~/composables/useAudioRecorder'

const { recipeId, recipeName, ingredients, steps } = defineProps<{
  recipeId: string
  recipeName: string
  /** Sichtbare Zeilen in Anzeige-Reihenfolge — daraus entsteht der `idx`. */
  ingredients: readonly RecipeIngredientSource[]
  steps: readonly RecipeStepSource[]
}>()

const emit = defineEmits<{ apply: [payload: AiRecipeEditApplyPayload] }>()
...
const proposal = ref<EditedRecipe | null>(null)
const ingredientDiff = ref<ListDiffEntry[]>([])
const stepDiff = ref<ListDiffEntry[]>([])
const selectedIngredients = ref<Set<number>>(new Set())
...
/** Der Rezeptname ändert sich mit? Wird als Banner über dem Diff gezeigt. */
const nameChanged = computed(() => {
  const target = proposal.value
  return target !== null && target.name.trim() !== recipeName
})

function countSelected(diff: readonly ListDiffEntry[], selected: ReadonlySet<number>): number {
  return diff.filter((entry, index) => selected.has(index) && entry.kind !== 'unchanged').length
}

/** Nur echte Änderungen zählen für „Übernehmen (N)" — Unverändertes nicht. */
const selectedChangeCount = computed(() =>
  countSelected(ingredientDiff.value, selectedIngredients.value)
  + countSelected(stepDiff.value, selectedSteps.value),
)

const totalChangeCount = computed(() =>
  ingredientDiff.value.filter(entry => entry.kind !== 'unchanged').length
  + stepDiff.value.filter(entry => entry.kind !== 'unchanged').length,
)

const hasAnyChange = computed(() => {
  const target = proposal.value
  if (target === null) return false
  return hasRecipeChanges(ingredientDiff.value, stepDiff.value, recipeName, target)
})
...
const chips = computed<DiffChip[]>(() => {
  const count = (kind: ListDiffEntry['kind']): number =>
    countByKind(ingredientDiff.value, selectedIngredients.value, kind)
    + countByKind(stepDiff.value, selectedSteps.value, kind)
...
function countByKind(
  diff: readonly ListDiffEntry[],
  selected: ReadonlySet<number>,
  kind: ListDiffEntry['kind'],
): number {
  return diff.filter((entry, index) => entry.kind === kind && selected.has(index)).length
}

/**
 * Momentaufnahme der Zeilen zum Zeitpunkt des Absendens. Der Server rechnet
 * bis zu fünf Minuten; währenddessen kann der Sync das Rezept verändern —
 * der `idx` der Antwort zählt aber in GENAU diese Arrays.
 */
interface Snapshot {
...
function snapshotRows(): Snapshot {
  return {
    ingredients: ingredients.map(row => ({ id: row.id, name: row.name, quantity: row.quantity })),
    steps: steps.map(row => ({ id: row.id, description: row.description })),
  }
}
...
function finish(result: AiResult<EditedRecipe>, snapshot: Snapshot): void {
  controller = null
  ...
  ingredientDiff.value = computeListDiff(ingredientDiffSource(snapshot.ingredients), editedIngredientsAsItems(result.value))
  stepDiff.value = computeListDiff(stepDiffSource(snapshot.steps), editedStepsAsItems(result.value))
  selectedIngredients.value = defaultDiffSelection(ingredientDiff.value)
  selectedSteps.value = defaultDiffSelection(stepDiff.value)
  phase.value = 'preview'
}
...
async function sendPrompt(): Promise<void> {
  ...
  const active = begin()
  const result = await requestEditRecipe(
    {
      targetId: recipeId,
      prompt: text,
      currentRecipe: buildCurrentRecipePayload(recipeName, snapshot.ingredients, snapshot.steps),
    },
    active.signal,
  )
...
async function toggleRecording(): Promise<void> {
  ...
  const snapshot = snapshotRows()
  const active = begin()
  const result = await requestVoiceEditRecipe(
    recording,
    {
      targetId: recipeId,
      currentRecipe: buildCurrentRecipePayload(recipeName, snapshot.ingredients, snapshot.steps),
...
function toggledSet(current: ReadonlySet<number>, index: number): Set<number> {
...
function toggleIngredient(index: number): void {
  selectedIngredients.value = toggledSet(selectedIngredients.value, index)
}

function toggleStepEntry(index: number): void {
  selectedSteps.value = toggledSet(selectedSteps.value, index)
}

function discardPreview(): void {
  proposal.value = null
  ingredientDiff.value = []
  stepDiff.value = []
  selectedIngredients.value = new Set()
  selectedSteps.value = new Set()
  phase.value = 'input'
}
...
function applySelection(): void {
  ...
  const pick = (diff: readonly ListDiffEntry[], selected: ReadonlySet<number>): ListDiffEntry[] =>
    diff.filter((entry, index) => selected.has(index) && entry.kind !== 'unchanged')

  emit('apply', {
    name: target.name,
    ingredientEntries: pick(ingredientDiff.value, selectedIngredients.value),
    stepEntries: pick(stepDiff.value, selectedSteps.value),
  })
  open.value = false
}
...
/**
 * `withQuantity` unterscheidet die Blöcke: Zutaten zeigen Mengen-Details,
 * Schritte nicht (ihre „Menge" ist eine Konstante des Adapters).
 */
function rowView(entry: ListDiffEntry, withQuantity: boolean): DiffRowView {
  switch (entry.kind) {
    case 'added':
      return {
        prefix: '+',
        ...
        background: '#EFFAF5',
        name: entry.name,
        detail: withQuantity && entry.quantity > 1 ? `${entry.quantity}×` : null,
        strike: false,
        dimmed: false,
      }
    case 'removed':
    ...
    case 'modified':
      return {
        ...
        background: '#FFF7ED',
        name: entry.name,
        detail: withQuantity ? quantityDetail(entry.oldQuantity, entry.newQuantity) : null,
        strike: false,
        dimmed: false,
      }
    case 'renamed': {
      const parts = [
        ...
        withQuantity ? quantityDetail(entry.oldQuantity, entry.newQuantity) : null,
...
const ingredientRows = computed(() => ingredientDiff.value.map(entry => ({
  unchanged: entry.kind === 'unchanged',
  view: rowView(entry, true),
})))

const stepRows = computed(() => stepDiff.value.map(entry => ({
  unchanged: entry.kind === 'unchanged',
  view: rowView(entry, false),
})))
</script>
...
<template>
  ...
  >
    ...
    >
      ...
      <UTextarea
        v-model="prompt"
        placeholder="Mach das Rezept vegetarisch"
        :rows="3"
        size="xl"
    ...
    <AiLoadingState
      ...
      :phrases="EDIT_RECIPE_PHRASES"
      @cancel="cancelRequest"
    />

    <!-- Phase 3: Diff-Vorschau, zwei Blöcke -->
    <div
      v-else
      class="flex flex-col gap-3"
    >
      ...
      <div
        v-else
        class="flex max-h-[50vh] flex-col gap-3 overflow-y-auto"
      >
        <section
        ...
        >
          <h3
            class="px-1 text-[0.875rem] font-bold"
            style="color: var(--md-on-surface-variant)"
          >
            Zutaten
          </h3>
          ...
          >
            ...
            <span class="flex shrink-0 items-center pr-3">
              <UCheckbox
                :model-value="selectedIngredients.has(index)"
        ...
        >
          ...
          >
            ...
            <span class="flex shrink-0 items-center pr-3">
              <UCheckbox
                :model-value="selectedSteps.has(index)"
                :aria-label="`${row.view.name} übernehmen`"
                @update:model-value="toggleStepEntry(index)"
              />
            </span>
          </label>
        </section>
